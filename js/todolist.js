...
(() => {
    ...
    document.onreadystatechange = () => {
        ...
        Ajax.sendGetRequest(API_URL, {id:1}, MediaFormat.JSON,(callbackSuccess) =>{
            loadTasks(callbackSuccess);
        } , (callbackError) => {showError(1, 'No ha podido regargar la pagina.') }, true);
    };
...
*/
    ...
    const addTask = (e) => {
        ...
        //(url, params, accept, callbackSuccess, callbackError, asynchronous, type)
        Ajax.sendPostRequest(API_URL, task, MediaFormat.JSON,(callbackSuccess) =>{
            addTaskToList(task);
            newTaskInput=newTaskInputOther;
        } , (callbackError) => {showError(1, 'No ha podido agregar la tarea.') }, true,MediaFormat.JSON);
    ...
    const addOnChangeEvent = (task) => {
        ...
        checkBox.onchange = (e) => {
            if(e.target.checked==true){// leer el nuevo estado de la tarea (que solo puede ser TERMINADO(true) or PENDIENTE(false))
                document.getElementById(`task-${task.id}`).querySelector('label > input')='false';
                document.getElementById(`task-${task.id}`).checked = false;
                addTaskToList(task);
                Ajax.sendPutRequest(API_URL+'/'+task.id, task, MediaFormat.JSON,(callbackSuccess) =>{
                    revertHTMLChangeOnEdit('TEMRINADO');
                } , (callbackError) => {showError(1, 'No ha podido modificar el estado de la tarea.') }, true);
            }else{
                document.getElementById(`task-${task.id}`).querySelector('label > input')=true;
                addTaskToList(task);
                Ajax.sendPutRequest(API_URL+'/'+task.id, task, MediaFormat.JSON,(callbackSuccess) =>{
                    revertHTMLChangeOnEdit('PENDIENTE');
                } , (callbackError) => {showError(1, 'No ha podido modificar el estado de la tarea.') }, true);
            }
    ...
    const editTask = (e) => {
        ...
        buttonOK.onclick = () => {
            ...
            Ajax.sendPutRequest(API_URL+'/'+id, currentTask, MediaFormat.JSON,(callbackSuccess) =>{
                ...
                return customResult;
            } , (callbackError) => {showError(1, 'No ha podido modificar la tarea.') }, false);
    ...
    const removeTaskFromList = (id) => {
        ...
        var imagen =document.getElementById(`task-${task.id}`);
        if (!imagen){
    ...
    const removeTask = (e) => {
        const id = e.target.dataset.id;
        Ajax.sendDeleteRequest(API_URL+'/'+id, id, MediaFormat.JSON,(callbackSuccess) =>{
            removeTaskFromList(id);
        } , (callbackError) => {showError(1, 'No ha podido eliminar la tarea.') }, true);
       // (url, params, accept, callbackSuccess, callbackError, asynchronous) 
